...
3. 搭建C端的文生图**应用**
   * 使用小程序前端
   * 后端使用GO语言实现，调用腾讯混元API返回BASE64编码
   *  使用腾讯云对象存储 (COS）

界面效果：

![1764483304625](assets/1764483304625.png)

### 额外收获

在push仓库到github的时候，提示上传了腾讯的密钥所以不给push。

所以使用rebase进行修复，将密钥删除才行。git真的是智能啊！

第一次使用命令行的方式修复冲突问题，去掉了GUI返回本源。
